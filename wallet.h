...
{
    // random byte using Mersenne Twister.
...
struct Transaction
{
    ...
    __uint128_t getTrTimestamp()
    {
        return std::chrono::duration_cast<std::chrono::duration<
            __uint128_t>>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()))
            .count();
    }
...
public:
    ...
    GenerateNewWalletAddress(std::string askForPrivKey = "")
    {
        ...
        std::string AESkeyStr = "";
        for (int c = 0; c < 32; c++)
        { /* plain text = new AES key in string */
        ...
        {
            std::cout << std::endl
                      << "AES256 key 1:\t" << aesKeyToStr<uint8_t>(AESkey);
            std::cout << std::endl
                      << std::endl;
...
public:
    ...
    std::pair<std::shared_ptr<uint64_t>, std::vector<std::shared_ptr<uint8_t>>>
    newTransaction(std::shared_ptr<uint64_t> sender, std::shared_ptr<uint64_t> receiver, uint32_t amount, std::vector<std::shared_ptr<uint64_t>> &mempool, std::map<std::shared_ptr<uint64_t>, std::vector<std::shared_ptr<uint8_t>>> verifyInfo, std::string sellorbuy, std::vector<std::shared_ptr<uint64_t>> &transactionhashes,
                   std::map<std::string, std::shared_ptr<uint8_t>> &
                       transactionsEnc,
                   int32_t &storedCrypto, std::vector<std::shared_ptr<uint8_t>> AESkeysWallet, std::vector<uint32_t> &trnsLengths, std::vector<uint32_t> all_trns_lengths,
                   ...
                   std::string askForPrivKey = "")
    {
        ...
        if (walletAddress != nullptr)
        {
            ...
            transactionsEnc.insert(it, std::pair<std::string, std::shared_ptr<uint8_t>>(trns.encryptTr(newAES_TrKey, sellorbuy),
                                                                                        newAES_TrKey));
...
struct Wallet
{
    ...
    std::pair<std::shared_ptr<uint64_t>, std::vector<std::shared_ptr<uint8_t>>>
    new_transaction(std::shared_ptr<uint64_t> sender, std::shared_ptr<uint64_t> receiver, uint32_t amount, std::vector<std::shared_ptr<uint64_t>> mempool, std::string sellorbuy, std::vector<std::shared_ptr<uint64_t>> transactionhashes,
                    std::map<std::string, std::shared_ptr<uint8_t>> &
                        transactionsEnc,
                    int32_t storedCrypto, std::vector<uint32_t> trnsLengths, std::vector<uint32_t> all_trns_lengths,
                    std::string askForPrivKey = "")
    {
        Address address = Address();
