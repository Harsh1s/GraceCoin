...
namespace MerkleTree
{
    ...
    {
    private:
        std::vector<std::shared_ptr<uint64_t>> append_level(std::vector<std::shared_ptr<uint64_t>>
                                                                Plevel,
                                                            uint64_t len)
        {
            SHA512 hash = SHA512();
    ...
    public:
        std::shared_ptr<uint64_t> append_levels(std::vector<std::shared_ptr<uint64_t>> mempool,
                                                uint64_t len, std::shared_ptr<uint64_t> merkle_root)
        {
            ...
            if (level.size() == 1)
            {
                merkle_root = std::move(std::shared_ptr<uint64_t>(level[0]));
            }
            return merkle_root;
        }
    };

    inline std::shared_ptr<uint64_t> merkleRoot(std::vector<std::shared_ptr<uint64_t>> Mempool)
    {
        Node node = Node();
