...
namespace Blockchain
{
    ...
    {
        // Random numerical type using Mersenne Twister.
    ...
    inline uint64_t calchashRateSingle()
    {
        ...
        } while (std::chrono::duration_cast<std::chrono::seconds>(end_t - start).count() != 1);
        return hashes.size();
    }
...
protected:
    ...
    mineSingleTr(std::string encryptedTr, std::shared_ptr<uint8_t> key,
                 uint64_t difficulty, std::vector<std::shared_ptr<uint64_t>> mempool, uint64_t nonce, uint64_t trnsLength)
    {
        ...
        for (int i = 0; i < mempool.size(); i++)
        {
            ...
            for (int c = 0; c < 8; c++)
            {
                if (mempool[i].get()[c] == hash.get()[c])
                { // if any index of mempool matches hash
        ...
        std::cout << "microseconds it took to verify transaction: "
                  << std::dec << std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
...
public:
    ...
    mineBlock(const std::map<std::string, std::shared_ptr<uint8_t>> encryptedTs,
              uint64_t blockNonce, uint64_t difficulty, std::vector<std::shared_ptr<uint64_t>> mempool, std::shared_ptr<uint64_t> v_merkle_root, std::vector<uint32_t> trnsLengths)
    {
...
public:
    ...
    std::shared_ptr<uint64_t> genBlock(std::shared_ptr<uint64_t> target,
                                       uint64_t nonce, std::shared_ptr<uint64_t> merkle_root, double difficulty)
    {
        std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
        bool valid;
        ...
        for (int c = 0; c < 8; c++)
        {
            while (target.get()[c] >= pow(2, 48))
            {
                target.get()[c] = sha512(merkle_root_str +
                                         std::to_string(newNonce + difficulty))
                                      .get()[c];
                newNonce++;
            }
        }
    ...
    std::tuple</* prevBlockHash */ std::shared_ptr<uint64_t>,
               ...
               /* average hashrate */ double>
    data(std::vector<std::shared_ptr<uint64_t>> mempool)
    {
    ...
    std::string data_str(std::shared_ptr<uint64_t> prevBlockHash, std::string timestamp, uint32_t blockchainSize, uint64_t nonce,
                         double difficulty, double nextBlockGenTime,
                         double avHashrate, std::vector<std::shared_ptr<uint64_t>> clean_mempool, std::string blockchain_version)
    {
        /* use this to represent block in blockchain, use tuple data to
