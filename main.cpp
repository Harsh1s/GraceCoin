...
int main(int argc, char **argv)
{
    ...
    /* UI */
    std::vector<std::string> commandDescriptions{"help: show basic commands with descriptions",
    ...
    /* terminal UI */
    // About
    std::cout << "\n---- GraceCoin by Harsh Raj ----\n"
              << "This is a hobby project to explore my interest in Cryptocurrency and Blockchain.\n";
    ...
    if (argc != 1 && !console_ui_activate)
    {
        ...
        else if (argc >= 3 && strcmp(argv[argc - 1], "-help") == 0)
        {
            ...
            { // get char array input as string
                tmp += argv[c];
                if (c < argc - 1)
                {
            ...
            {
                if (commandDescriptions[c].starts_with(tmp.substr(0, tmp.length() - 6)) &&
                                              ...
                                              strcmp(argv[1], "e-wallet-aes256") == 0))
        {
            ...
            {
                if (argc == 2)
                { // if key generation not requested
        ...
        else if (argc == 2 && strcmp(argv[1], "decrypt-wallet") == 0)
        {
            ...
            if (walletMap.empty() && ciphertextW != "")
            {
                ...
                walletMap.insert(itWalletMap, std::pair<std::shared_ptr<uint64_t>,
                                                        std::vector<std::shared_ptr<uint8_t>>>(walletAddress, walletKeysDec));
                std::cout << "\nwallet data saved\n";
            }
            else
            {
